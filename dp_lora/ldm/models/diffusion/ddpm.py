...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if ckpt_path is not None:
            ...
            # self.instantiate_first_stage(first_stage_config) ### 应急添加
            self.restarted_from_ckpt = True ### 应急删除

        self.dp_config = dp_config
        # if dp_config and dp_config.enabled:
        #     self.privacy_engine = myPrivacyEngine()
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                if self.training:
                    for key in c:
                        if key == self.cond_stage_key:
                            c[key] = c[key].unsqueeze(1).repeat_interleave(self.DPDM_k, dim=1).reshape(-1, *c[key].shape[1:])
    ...
    def p_losses(self, x_start, cond, t, noise=None):  # Added k parameter
        # noise = default(noise, lambda: torch.randn_like(x_start))
        if self.training:
            x_start_extend = x_start.unsqueeze(1).repeat_interleave(self.DPDM_k, dim=1)
            x_start_extend = x_start_extend.reshape(-1, *x_start.shape[1:])
            noise = default(noise, lambda: torch.randn_like(x_start_extend))
            random_t = torch.randint(0, self.num_timesteps, (x_start_extend.shape[0],), device=self.device).long()
            x_noisy = self.q_sample(x_start=x_start_extend, t=random_t, noise=noise)
            model_output = self.apply_model(x_noisy, random_t, cond)
        else:
            random_t = torch.randint(0, self.num_timesteps, (x_start.shape[0],), device=self.device).long()
            noise = default(noise, lambda: torch.randn_like(x_start))
            x_noisy = self.q_sample(x_start=x_start, t=random_t, noise=noise)
            model_output = self.apply_model(x_noisy, random_t, cond)
        ...
        logvar_t = self.logvar[random_t.cpu()].to(self.device)
        ...
        loss_vlb = (self.lvlb_weights[random_t] * loss_vlb).mean()
